...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: SMTT / STPT GTFS (smtt.ro)' }));
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: SMTT / STPT (smtt.ro)';
  ...
  // NOT fitBounds(meta.bbox): the metropolitan lines reach Cruceni and
  // Seceani, ~30 km out, so fitting the data bbox would open on countryside
  // with the city as a small core. Open on central Timișoara instead.
  map.jumpTo({ center: [21.2260, 45.7540], zoom: 12.0 });
}
