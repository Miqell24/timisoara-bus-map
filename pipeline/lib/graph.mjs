...
// and so does one tagged railway=disused — the rails are physically there, the
// tag only says no service ran on the day someone surveyed it. OSM lags behind
// reopenings: Sofia's tram 6 was carrying service through Надежда while its
// ways still said construction=tram, which tore a 440 m hole in the line, and
// Belgrade's whole Novi Beograd corridor to Blok 45 sits under
// `disused:railway=tram` carrying `opening_date=2026-03-08` — a date already
// past — sourced to the operator's own service-change notice, while the feed
// schedules four tram lines over it every day. The road graph has accepted
// highway=construction from the start for the same reason; the GTFS shape is
// the evidence that service runs there.
// Both spellings of the lifecycle tag count: the old `construction=tram` and
// the prefixed `construction:railway=tram`. Belgrade mixes them WITHIN one
// corridor — the Blok 45 tracks say `construction=tram`, the 200 m ramp that
// reconnects them to the Sava bridge says `construction:railway=tram` — so
// reading only one spelling left the corridor stranded on its own island.
export function railKind(tags) {
  if (!tags) return undefined;
  if (tags.railway === 'construction') return tags.construction || tags['construction:railway'];
  if (tags.railway === 'disused') return tags['disused:railway'] || tags.disused;
  return tags.railway;
}
function tramAccess(tags) {
  ...
  // unreachable (proven in Vienna, where line 18 drew a 55 m chord across a
  // block). Allowing them also lowered the mean error across the tram network.
