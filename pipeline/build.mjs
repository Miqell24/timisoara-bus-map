...
// Timișoara: ONE SMTT feed (smtt.ro) split by route_type into two modes —
// buses+trolleybuses (3, 11) on the road graph and trams (0) on the tram
// graph. Trolleybuses ride the bus network in green per-line colors. There is
// no metro here, so the engine's metro treatment stays unused. The operator
// numbers its modes from ONE pool with no number used twice, so the line keys
// are bare numbers and need no mode prefixes.
// The feed's one route_type=4 line — V1, the Bega vaporetto — is NOT drawn:
// a boat has neither roadway nor track to match against, and Istanbul set the
// precedent when its ferries were built and then removed on request.
...
// ONE feed, TWO cfgs: routeTypes splits the SMTT bundle per mode (3 = bus and
// 11 = trolleybus, both riding the road network, trolleybuses in green;
// 0 = tram on the tram graph). Without the filter `--all` on the bus mode
// would swallow the tram lines too — and route_type 4, the V1 vaporetto, is
// listed in NEITHER cfg, which is what keeps the boat off the map.
// Timișoara has no metro, so the third cfg its siblings carry is absent here.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/timisoara.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['33']),
  feeds: [
    {
      tag: 'stpt', dir: 'data/gtfs', routeTypes: ['3', '11'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/timisoara-rail.json',
  ...
  feeds: [
    { tag: 'stpt', dir: 'data/gtfs', mapKey: (sn) => norm(sn), routeTypes: ['0'] },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      // red tram — across all fifteen maps. Timișoara's service classes stay
      // legible in the line number itself (E = express, M = metropolitan).
    ...
    for (const s of await readCsv(join(fdir, 'stops.txt'))) {
      ...
      // SMTT publishes its stop names properly cased and fully accented
